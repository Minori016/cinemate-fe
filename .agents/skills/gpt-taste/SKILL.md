...
- **Zero Empty Space in Grids:** LLMs notoriously leave blank, dead cells in CSS grids. You MUST use Tailwind's `grid-flow-dense` (`grid-auto-flow: dense`) or manually place cells with absolute precision.
- **Visual Contrast inside Bento Cells:** Every cell cannot be a white box with text. 
  - At least 1 cell must be dark background with glowing text.
  - At least 1 cell must house a high-end visual asset (image or mini interactive UI component).
  - Use asymmetric row and column spans (e.g. `col-span-2 row-span-1` next to `col-span-1 row-span-2`).
