...
import { CheckCircle2, XCircle, Loader2, ArrowRight, Calendar, Clock, MapPin, Tag, Film, Ticket, Download, Share2, Printer, Coffee } from 'lucide-react'
              ...
              <div className="bg-[#12141C] border border-white/10 border-t-0 rounded-b-3xl p-6 md:p-8">
                ...
                {bookingDetails.concessions && bookingDetails.concessions.length > 0 && (
                  <div className="mb-6 bg-white/5 rounded-xl p-4 border border-white/10">
                    <p className="text-gray-400 text-xs font-bold uppercase tracking-wider mb-2 flex items-center gap-1.5">
                      <Coffee size={14} /> Đồ ăn & Bắp nước đi kèm
                    </p>
                ...
                <div className="bg-white/5 rounded-xl p-4 border border-white/5 flex flex-col md:flex-row gap-4 items-center justify-between">
                  <p className="text-sm text-gray-400">
                    Vé điện tử đã được gửi tới email của bạn. Bạn chỉ cần xuất trình mã QR này tại cửa rạp để soát vé.
                  </p>
                  <img src={`https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${bookingDetails.id}&bgcolor=ffffff&color=000000`} alt="QR Code" className="w-20 h-20 rounded bg-white p-1 shrink-0" />
                </div>
              </div>
            </motion.div>
          )}
        ...
        >
          {success && (
            <>
              ...
              <button 
                onClick={handlePrint} 
                className="px-6 py-3.5 bg-white/10 border border-white/20 rounded-xl font-bold text-white hover:bg-white/20 transition-all duration-300 flex items-center justify-center gap-2 w-full sm:w-auto"
              >
                <Printer size={18} /> In Vé / Tải Xuống
              </button>
            </>
          )}
          <button 
