    ...
    <div className="min-h-screen bg-[#07080E] flex flex-col pt-20 pb-16 px-4 relative overflow-hidden font-sans text-gray-100">
      ...
      <div className="w-full max-w-5xl mx-auto z-10 flex flex-col items-center">
        ...
        <div className="w-full mb-6 no-print">
          <button
            onClick={() => navigate('/')}
            className="inline-flex items-center gap-2 text-xs font-bold text-gray-400 hover:text-white transition-colors bg-white/5 hover:bg-white/10 border border-white/10 px-3.5 py-2 rounded-xl cursor-pointer"
          >
        ...
        >
          <div className="relative mb-4">
            <div className={`absolute inset-0 blur-2xl ${success ? 'bg-emerald-500/40' : verificationState === 'pending' ? 'bg-amber-500/40' : 'bg-rose-600/40'} rounded-full`} />
            {success ? (
              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center text-white shadow-[0_0_40px_rgba(16,185,129,0.6)] relative z-10 border-2 border-emerald-400/40">
                <CheckCircle2 size={44} strokeWidth={2.2} />
              </div>
            ) : verificationState === 'pending' ? (
              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-amber-500 to-amber-700 flex items-center justify-center text-white shadow-[0_0_40px_rgba(245,158,11,0.6)] relative z-10 border-2 border-amber-400/40">
                <Loader2 size={44} className="animate-spin" strokeWidth={2.2} />
              </div>
            ) : (
        ...
        {!success && (
          <motion.div 
            ...
            className="w-full max-w-md bg-white/[0.03] border border-red-500/20 rounded-2xl p-6 text-center shadow-xl no-print mb-8"
          >
            ...
            <div className="flex gap-3 justify-center">
              <button 
                ...
                className="px-6 py-3 bg-red-600 hover:bg-red-500 text-white font-bold text-sm rounded-xl transition-all shadow-[0_0_20px_rgba(229,9,20,0.4)] cursor-pointer"
              >
                Đặt lại vé khác
              </button>
              <button 
                onClick={() => navigate('/')} 
                className="px-6 py-3 bg-white/10 hover:bg-white/20 text-white font-bold text-sm rounded-xl transition-all cursor-pointer"
              >
                Trang chủ
              </button>
            </div>
