...
export default function SeatSelectionPage() {
      ...
      <div className="fixed bottom-0 left-0 w-full z-30 p-4 md:p-6 pointer-events-none flex justify-center">
        <div className="pointer-events-auto w-full max-w-4xl bg-[#1a1c1c]/90 backdrop-blur-2xl rounded-2xl border border-white/10 shadow-[0_-10px_45px_rgba(229,9,20,0.25)] p-4 md:p-6 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3 flex-grow text-left">
            ...
            <p className="text-base md:text-lg font-bold text-white tracking-wide m-0">
              Bạn muốn xem phim? Đặt vé tại đây
            </p>
          </div>
          ...
          <div className="flex items-center gap-6 shrink-0">
            <button
              onClick={() => {
                navigate(`/movies/${movieId}?date=${dateStr}&time=${time}`)
              }}
              className="bg-[var(--color-primary)] text-white font-bold text-base px-10 py-4 rounded-xl shadow-[0_0_24px_rgba(229,9,20,0.35)] hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2 group uppercase tracking-wider cursor-pointer border-none"
            >
              <span>Đặt vé ngay</span>
              <span className="material-symbols-outlined text-lg font-black group-hover:translate-x-1 transition-transform">confirmation_number</span>
            </button>
          </div>
        </div>
      </div>
  ...
  function renderSeatButton(seat, type) {
    ...
    return (
      <div
        key={seat.id}
        className={`seat-btn w-8 h-8 rounded border flex items-center justify-center text-xs font-bold relative cursor-default select-none ${
          isOccupied ? 'occupied opacity-40' :
          isVip ? 'vip border-[#f59e0b]/60 text-[#f59e0b]' :
          'border-gray-600 text-gray-300'
        }`}
        title={seat.id}
      >
        {isVip && !isOccupied ? 'V' : seat.label}
      </div>
    )
  }
  ...
  function renderCoupleButton(seat) {
    ...
    return (
      <div
        key={seat.id}
        className={`seat-btn couple h-8 rounded border flex items-center justify-center text-xs font-bold relative cursor-default select-none ${
          isOccupied ? 'occupied opacity-40' :
