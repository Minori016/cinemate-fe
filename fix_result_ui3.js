...
if (content.match(regex)) {
    ...
    // try a simpler regex
    const regexSimple = /\{discountAmount > 0 && \(\r?\n\s*<div className="flex justify-between items-center text-emerald-400 font-medium">[\s\S]*?<\/span>\r?\n\s*<\/div>\r?\n\s*\)\}/;
    if (content.match(regexSimple)) {
        content = content.replace(regexSimple, replaceStr);
        fs.writeFileSync(file, content, 'utf8');
        console.log("CheckoutResultPage Mã giảm giá block updated (simple)");
    } else {
